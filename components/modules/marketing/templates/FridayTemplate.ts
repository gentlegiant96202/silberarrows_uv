...
export const generateFridayTemplate = (
...
${fontFaceCSS}
    ...
    <style>
      ...
      *:not(i):not(.fas):not(.far):not(.fab):not(.fal) { font-family: 'Resonate', 'Inter', sans-serif !important; }
      ...
      body { font-family: 'Resonate', 'Inter', sans-serif; background: linear-gradient(135deg, #065f46, #10b981); color: #ffffff; height: 100vh; display: flex; align-items: center; justify-content: center; text-align: center; }
      ...
      .fun-facts { display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 25px; }
      .fact { background: rgba(255,255,255,0.15); padding: 25px; border-radius: 12px; }
    </style>
    
    <div class="content">
      <div class="badge">🎉 ${formData.badgeText || 'FUN FRIDAY'}</div>
      ...
      <div class="fun-facts">
        <div class="fact">🚗<br>Amazing Facts</div>
        <div class="fact">⚡<br>Did You Know?</div>
      </div>
