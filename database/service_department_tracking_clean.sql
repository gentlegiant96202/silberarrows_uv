-- Service Department Performance Tracking System (Clean Design)
-- Two tables: targets + normalized metrics

-- 0. CLEANUP: Drop old tables and functions if they exist
...
-- 2. SERVICE METRICS (Both inputs and calculated metrics)
...
-- 3. METRIC DEFINITIONS (What each metric means)
CREATE TABLE IF NOT EXISTS service_metric_definitions (
    ...
    unit TEXT,
    calculation_formula TEXT, -- For calculated metrics
    display_order INTEGER DEFAULT 0,
    is_active BOOLEAN DEFAULT true
);

-- 4. INSERT METRIC DEFINITIONS
INSERT INTO service_metric_definitions (metric_name, display_name, description, metric_type, metric_category, unit, display_order) VALUES
-- INPUT METRICS
('working_days_elapsed', 'Working Days Elapsed', 'Number of working days passed this month', 'count', 'input', 'days', 1),
('current_net_sales', 'Current Net Sales', 'Cumulative net sales so far this month', 'currency', 'input', 'AED', 2),
('current_net_labor_sales', 'Current Net Labor Sales', 'Cumulative labor sales so far this month', 'currency', 'input', 'AED', 3),
('number_of_invoices', 'Number of Invoices', 'Total invoices issued so far this month', 'count', 'input', 'units', 4),
('current_marketing_spend', 'Current Marketing Spend', 'Total marketing spend so far this month', 'currency', 'input', 'AED', 5),

-- CALCULATED METRICS
('current_net_sales_percentage', 'Current Net Sales Percentage', 'Percentage of net sales target achieved', 'percentage', 'calculated', '%', 6),
('current_labour_sales_percentage', 'Current Labour Sales Percentage', 'Percentage of labor sales target achieved', 'percentage', 'calculated', '%', 7),
('remaining_net_sales', 'Remaining Net Sales', 'Amount needed to reach net sales target', 'currency', 'calculated', 'AED', 8),
('remaining_labour_sales', 'Remaining Labour Sales', 'Amount needed to reach labor sales target', 'currency', 'calculated', 'AED', 9),
('current_daily_average', 'Current Daily Average', 'Average daily net sales so far', 'currency', 'calculated', 'AED', 10),
('estimated_net_sales', 'Estimated Net Sales', 'Projected net sales based on current pace', 'currency', 'calculated', 'AED', 11),
('estimated_net_sales_percentage', 'Estimated Net Sales Percentage', 'Projected percentage of net sales target', 'percentage', 'calculated', '%', 12),
('estimated_labor_sales', 'Estimated Labor Sales', 'Projected labor sales based on current pace', 'currency', 'calculated', 'AED', 13),
('estimated_labor_sales_percentage', 'Estimated Labor Sales Percentage', 'Projected percentage of labor sales target', 'percentage', 'calculated', '%', 14),
('daily_average_needed', 'Daily Average Needed', 'Daily sales needed to reach target', 'currency', 'calculated', 'AED', 15)

ON CONFLICT (metric_name) DO NOTHING;

-- 5. FUNCTION TO CALCULATE AND STORE ALL METRICS FOR A DATE
...
-- 6. SAMPLE DATA - JANUARY 2025 TARGETS
...
ALTER TABLE service_metric_definitions ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "service_definitions_policy" ON service_metric_definitions FOR SELECT USING (true);
...
AS $$
    ...
    JOIN service_metric_definitions smd ON sm.metric_name = smd.metric_name
    WHERE sm.metric_date = target_date
...
AS $$
    ...
    AND (filter_metric_name IS NULL OR sm.metric_name = filter_metric_name)
    ORDER BY sm.metric_date, smd.display_order;
$$;

SELECT 'Service Department tracking system (clean design) created successfully!' as status; 