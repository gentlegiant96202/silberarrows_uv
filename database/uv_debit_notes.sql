...
-- 
-- Debit notes INCREASE what customer owes (opposite of credit notes)
-- Use cases: Additional charges, price adjustments, corrections
-- 
-- Formula: balance_due = total_amount + debit_note_total - credit_note_total - paid_amount
-- Status: 'paid' when (paid_amount + credit_note_total) >= (total_amount + debit_note_total)
-- =====================================================

-- 1. Add 'debit_note' to adjustment_type enum
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'debit_note' AND enumtypid = 'uv_adjustment_type'::regtype) THEN
        ALTER TYPE uv_adjustment_type ADD VALUE 'debit_note';
    END IF;
END $$;
