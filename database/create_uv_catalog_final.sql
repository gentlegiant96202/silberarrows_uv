-- =====================================================
-- UV CATALOG TABLE - FINAL SAFE MIGRATION
-- =====================================================
...
-- Add unique constraint on car_id if it doesn't exist
DO $$ 
BEGIN
    -- Check if constraint exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'uv_catalog_car_id_key' 
        AND table_name = 'uv_catalog'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_year' 
        AND table_name = 'uv_catalog'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_mileage' 
        AND table_name = 'uv_catalog'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_price' 
        AND table_name = 'uv_catalog'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'valid_attempts' 
        AND table_name = 'uv_catalog'
    ) THEN
...
-- Success message
SELECT 'UV Catalog table migration completed successfully!' as result,
       (SELECT COUNT(*) FROM uv_catalog) as cars_populated; 