...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!invoiceId || !customerEmail || !customerName || !companySignerEmail || !pdfUrl) {
    ...
    const documentTitle = `Tax Invoice ${invoiceNumber || ''}`.trim();
    ...
    // Update the invoice with DocuSign envelope ID
    ...
    if (updateError) {
      console.error('Failed to update invoice with envelope ID:', updateError);
    }

    return NextResponse.json({
      success: true,
      envelopeId,
      message: `Invoice sent to ${companySignerEmail} for company approval. Customer will be notified after company signature is completed.`
    });
