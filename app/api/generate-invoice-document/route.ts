...
function generateInvoiceHTML(formData: any, lineItems: LineItem[], logoSrc: string) {
    ...
    <body>
      ...
      <div class="page first-page">
        ...
        <div class="content-container">
          ${formData.status === 'paid' ? '<div class="paid-stamp">PAID</div>' : ''}
          ...
          <div class="section">
            <table class="form-table">
              <tr>
                ...
                <td class="data">${formatDate(formData.invoiceDate)}</td>
                <td class="label">Invoice No.:</td>
                <td class="data">${safeString(formData.invoiceNumber)}</td>
              </tr>
              <tr>
                <td class="label">Sales Executive:</td>
                <td class="data">${safeString(formData.salesExecutive)}</td>
                <td class="label">Sales Order No.:</td>
                <td class="data">${safeString(formData.salesOrderNumber)}</td>
              </tr>
            </table>
          </div>
          ...
          <div class="section">
            <div class="text-content" style="margin: 0 0 8px 0;">
              This invoice relates to and completes the vehicle sale previously agreed. I confirm receipt of the above vehicle and acknowledge that the purchase has been completed in accordance with this invoice.
            </div>
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[generate-invoice-document] Request received:', { invoiceId });
    ...
    // Fetch invoice data
    console.log('[generate-invoice-document] Fetching invoice...');
    ...
    if (invError || !invoice) {
      console.error('[generate-invoice-document] Invoice fetch error:', invError);
      return NextResponse.json(
        { error: 'Invoice not found', details: invError?.message },
        { status: 404 }
      );
    }
    
    console.log('[generate-invoice-document] Invoice found:', invoice.invoice_number);

    // Fetch sales order data
    console.log('[generate-invoice-document] Fetching sales order...');
    ...
    if (soError || !salesOrder) {
      console.error('[generate-invoice-document] Sales order fetch error:', soError);
      return NextResponse.json(
    ...
    // Fetch line items from sales order
    console.log('[generate-invoice-document] Fetching line items...');
    ...
    if (liError) {
      console.error('[generate-invoice-document] Line items fetch error:', liError);
      return NextResponse.json(
    ...
    console.log('[generate-invoice-document] Line items found:', lineItems?.length || 0);
    ...
    console.log('[generate-invoice-document] Generating HTML...');
    const htmlContent = generateInvoiceHTML(formData, lineItems || [], logoSrc);
    console.log('[generate-invoice-document] HTML generated, length:', htmlContent.length);

    // Call PDFShift API
    console.log('[generate-invoice-document] Calling PDFShift API...');
    ...
    if (!pdfShiftResponse.ok) {
      ...
      console.error('[generate-invoice-document] PDFShift error:', pdfShiftResponse.status, errorText);
      throw new Error(`PDFShift API Error: ${pdfShiftResponse.status} - ${errorText}`);
    }

    console.log('[generate-invoice-document] PDF generated successfully');
    const pdfBuffer = await pdfShiftResponse.arrayBuffer();
    console.log('[generate-invoice-document] PDF size:', pdfBuffer.byteLength, 'bytes');
    
    // Upload to Supabase storage
    const fileName = `invoice-${invoiceId}-${Date.now()}.pdf`;
    console.log('[generate-invoice-document] Uploading to storage:', fileName);
    ...
    if (uploadError) {
      console.error('[generate-invoice-document] Upload error:', uploadError);
      throw new Error(`Failed to upload PDF: ${uploadError.message}`);
    }
    
    console.log('[generate-invoice-document] Upload successful');
