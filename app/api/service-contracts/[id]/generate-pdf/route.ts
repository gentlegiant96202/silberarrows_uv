...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Update contract with PDF URL and reset signing status (permanent fix)
    // Dynamically check which columns exist and update accordingly
    ...
    const { data: columns, error: columnError } = await supabase
      ...
      .in('column_name', ['signing_status', 'docusign_envelope_id', 'signed_pdf_url', 'sent_for_signing_at']);

    if (columnError) {
      console.error('⚠️ Could not check table columns:', columnError);
    }

    const existingColumns = columns?.map(col => col.column_name) || [];
    console.log('📋 Existing DocuSign columns in', tableName + ':', existingColumns);
    ...
    // Only add signing fields if they exist in the database
    if (existingColumns.includes('signing_status')) {
      updateData.signing_status = 'pending';
    }
    if (existingColumns.includes('docusign_envelope_id')) {
      updateData.docusign_envelope_id = null;
    }
    if (existingColumns.includes('signed_pdf_url')) {
      updateData.signed_pdf_url = null;
    }
    if (existingColumns.includes('sent_for_signing_at')) {
      updateData.sent_for_signing_at = null;
    }
    ...
    if (updateError) {
      ...
      // If update fails, try with just basic fields as fallback
      console.log('🔄 Attempting fallback update with basic fields only...');
      ...
      if (fallbackError) {
        ...
        throw new Error(`Database update failed: ${updateError.message}. Fallback also failed: ${fallbackError.message}`);
      } else {
        console.log('✅ Fallback update successful - PDF URL updated');
      }
    } else {
      const resetFields = existingColumns.length > 0 ? ' and signing status reset' : '';
      console.log('✅ Contract updated with PDF URL' + resetFields);
    }
    ...
    try {
      await supabase
        ...
        .insert({
          ...
          activity_description: `PDF document regenerated by ${validation.user.email}${existingColumns.length > 0 ? ' - signing fields reset' : ''}`,
          activity_data: {
            filename: filename,
            generated_by: validation.user.email,
            generated_at: new Date().toISOString(),
            pdf_url: publicUrl,
            signing_fields_available: existingColumns,
            signing_fields_reset: existingColumns.length > 0
          }
        });
    } catch (activityError) {
