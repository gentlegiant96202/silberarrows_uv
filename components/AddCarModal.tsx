...
export default function AddCarModal({ onClose, onCreated }: Props) {
  ...
  // 5 steps always: 0 Chassis, 1 Vehicle, 2 Pricing & Condition, 3 Specs, 4 Key Equipment/Description
  const totalSteps = 5;
  ...
  const validateStep = (): string[] => {
    ...
    // no required fields in step 3 (specs) beyond optional
  ...
  const handleNext = async () => {
    ...
    try {
          ...
          if (res.ok) {
            ...
            try {
              ...
              if(enrJson.success){
                ...
                // Only update if we actually got values from GPT
                setForm(prev=>({
                  ...prev,
      ...
      // advance once processing for current step completes
      setStep((s) => s + 1);
    } finally {
      setProcessing(false);
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const upperCaseFields = ['chassis_number', 'stock_number', 'vehicle_model', 'colour', 'interior_colour', 'key_equipment'];
    ...
    setForm((prev) => {
      ...
      if (name === 'chassis_number') {
        // Auto-derive stock number as last 6 characters (uppercase, trimmed)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.stock_number || !form.model_year || !form.vehicle_model || !form.colour || !form.chassis_number || !form.advertised_price_aed) return;
    setSaving(true);
    ...
    setSavedCar(data);
    // fetch documents list (likely empty)
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Consignment Customer fields at bottom */}
            ...
            <div className="flex gap-2 justify-between">
              ...
              {step < totalSteps - 1 && (
                <button
                  ...
                  disabled={processing}
                  className={
                    `flex-1 py-2 rounded text-white text-xs ` +
                    (processing ? 'bg-brand/60 cursor-wait' : 'bg-brand hover:bg-brand/90')
                  }
                >
          ...
          <div className="space-y-4">
            ...
            {docs.length > 0 && (
              <ul className="list-disc list-inside text-white/70 text-xs space-y-1">
                {docs.map((d:any)=> (
                  <li key={d.id}><a href={d.url} target="_blank" className="underline">Document {d.id.slice(0,4)}</a></li>
                ))}
              </ul>
            )}
