"use client";
import { X, Download, FileText, User, Building, Car, Calendar, Clock, ExternalLink, AlertTriangle, Edit, Save, Upload, Trash2, AlertCircle, Shield, Eye } from 'lucide-react';
        ...
        <div className="flex items-start justify-between mb-6 pr-8 gap-4 flex-wrap">
          <div className="flex flex-col">
            <h2 className="text-2xl font-bold text-white">
              {contract?.contract_type === 'warranty' 
                ? (isEditing ? 'Edit Warranty Agreement' : 'Warranty Agreement') 
                : (isEditing ? 'Edit ServiceCare Agreement' : 'ServiceCare Agreement')
              }
            </h2>
          ...
          <div className="flex gap-1.5 mt-0.5">
            {!isEditing ? (
              <>
                ...
                <button 
                  onClick={handleEdit}
                  className="flex items-center space-x-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded text-white transition-all duration-200"
                >
                ...
                <div className="space-y-3">
                  {contract.workflow_status === 'sent_for_signing' || contract.workflow_status === 'card_issued' ? (
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        // Update workflow status to sent_for_signing
                        try {
                          const headers = await getAuthHeaders();
                          const response = await fetch(`/api/service-contracts/${contract.id}`, {
                            method: 'PUT',
                ...
                <div className="flex items-center justify-center">
                  {contract.workflow_status === 'card_issued' ? (
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        // Update workflow status to card_issued
                        try {
                          const headers = await getAuthHeaders();
                          const response = await fetch(`/api/service-contracts/${contract.id}`, {
                            method: 'PUT',
