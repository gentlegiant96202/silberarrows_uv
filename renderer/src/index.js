...
function fillTemplate({ carDetails, pricing, firstImageUrl, secondImageUrl }) {
  ...
  // Generate monthly payment HTML with dirham symbol  
  const monthlyPaymentHtml = pricing.isCashOnly 
    ? 'CASH ONLY'
    : `<span>From</span>
            <span class="dirham-symbol">
              <svg viewBox="0 0 344.84 299.91" xmlns="http://www.w3.org/2000/svg">
            ...
            <span>${Number(pricing.monthlyPayment ?? 0).toLocaleString()}/mo</span>`;
  ...
  const replacements = {
    ...
    '{{monthlyPaymentHtml}}': monthlyPaymentHtml,
