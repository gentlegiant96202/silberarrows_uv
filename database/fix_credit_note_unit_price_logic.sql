-- 🔧 FIX CREDIT NOTE UNIT PRICE LOGIC
-- This fixes the unit price calculation for credit notes to maintain quantity consistency

-- Step 1: Update the credit note function with proper unit price logic
...
BEGIN
    ...
    LOOP
        ...
        -- Calculate proportional unit price for credit note
        -- If partial credit, adjust unit price proportionally
        IF v_credit_amount < v_charge.total_amount THEN
            v_credit_unit_price := -(v_charge.unit_price * (v_credit_amount / v_charge.total_amount));
        ELSE
            v_credit_unit_price := -v_charge.unit_price;
        END IF;
        ...
        INSERT INTO ifrs_lease_accounting (
            ...
            'credit_note',
            v_charge.quantity, -- Use original quantity
            v_credit_unit_price, -- Use calculated proportional unit price
        ...
        IF v_charge.vat_applicable AND v_charge.charge_type <> 'vat' THEN
            INSERT INTO ifrs_lease_accounting (
                ...
                'vat',
                v_charge.quantity, -- Use original quantity
                -(v_credit_amount * 0.05 / v_charge.quantity), -- Proportional VAT unit price
...
-- Step 2: Also fix the unit price constraint to be more flexible
...
ADD CONSTRAINT valid_unit_price CHECK (
    ...
    OR (charge_type = 'vat' AND (
        ...
        (credit_note_id IS NOT NULL AND (unit_price IS NULL OR unit_price <= 0))
        OR 
        -- VAT can be positive if it's a regular charge
        (credit_note_id IS NULL AND (unit_price IS NULL OR unit_price >= 0))
    ))
    OR (charge_type NOT IN ('refund', 'credit_note', 'vat') AND (unit_price IS NULL OR unit_price >= 0))
);

SELECT '✅ Credit note unit price logic fixed!' as result;
